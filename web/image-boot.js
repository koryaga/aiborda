// Runs inside the image and in the tests. There are no imports: the text of
// this file is inlined into the srcdoc by a plain script tag, so only
...
function createImage(doc, send, opts) {
  ...
  let execDepth = 0;
  // trustSync is true only in the synchronous + microtask tail right after the
  // model's code starts (before the first real trip through a macrotask).
  // While that holds, no human can wedge in — the tab is busy with synchronous
  // JS. Mutations seen in that window are certainly the model's, and their
  // nodes go into modelTouched. After the first macrotask (an await inside the
  // model's code, or the whole grace tail) execDepth > 0 can no longer be
  // trusted on its own: a human could have wedged in. From then on only writes
  // to nodes already known to be the model's count as the model's.
  let trustSync = false;
  ...
  // M9: a path built from an id is trusted only if it unambiguously leads back
  // to the same node — otherwise a duplicated or syntactically broken id (a
  // space, a leading digit) yields a wrong or unusable selector.
  function path(n) {
    if (n.nodeType === 3) n = n.parentNode;
    if (!n || n === doc.documentElement) return 'html';
    // M8: a detached node has no reliable path — lying about its address (a
    // bare tag can point at a different live node) is worse than flagging it.
    if (!n.isConnected) return n.nodeName.toLowerCase() + ' (removed)';
    if (n.id) {
      let unique = false;
  ...
  // session. All that does is drown the signal.
  ...
  function serialize(n) {
    ...
    // I7: a diff line is one line. Only newlines (and the spaces around them)
    // are collapsed; indentation inside a line is left alone.
  ...
  // Clearing the input field after a send. It lives here rather than in the
  // shell so that it is atomic with building the diff: as a separate message
  // the human could type something in between, and we would erase it.
  ...
  // Synchronisation is done on a clone: the live DOM is the agent's memory, and
  // mutating it for the sake of serialisation is not allowed.
  function snapshot() {
  ...
  // Attribute and text writes accumulate per key (node [+ attribute]):
  // - old — the oldValue of the very first HUMAN write for that key;
  // - override — if a later MODEL write followed a human one, override is its
  //   oldValue: exactly what the human left before the model overwrote the
  //   value (C2). A new human write resets override again.
  // - hasHuman — whether a human was involved at all; without that the line is
  //   not emitted.
  ...
  // returns the current value, not the value at the time of the mutation. The
  // old value comes from the earliest human write, the new one from the live
  // DOM — unless the model overwrote the value later (see applyRecord).
  function buildDiff() {
    const attrs = new Map();
    const texts = new Map();
    const removed = [];
    ...
    for (const r of recs) {
      if (r.type === 'attributes') {
        ...
        // same node — only the last one is kept (a Map, not an array).
        for (const n of r.added) added.set(n, r.target);
      }
    }

    // M10: a node added within this same window is described once, through
    // serialize() in the "added" line — there must be no separate lines about
    ...
    // without a repeated focus (focus never left, after all) has nothing to
    // compare against.
    ...
    for (const [node, entry] of texts) {
      ...
      if (entry.old === now) continue;
      // Only when both sides are whitespace: "\n " -> "hello" is a real edit.
  ...
  function install() {
    ...
    // the shell — they are different origins — so the image reports the press
    // itself. trusted() filters out synthetic events: otherwise the model's
    // code could start a turn on the human's behalf just by dispatching
    // keydown.
