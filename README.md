...
111 тестов на `node:test`. Тесты образа выполняют **тот же текст**
...
- **Индикатор показывает сырые имена событий** сессии — `turn_start`,
  `message_update`. Работает, но выглядит как отладка; текст модели при этом
  виден в выводе `node`, а не в странице.
